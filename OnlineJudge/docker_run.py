...
#     content = file.read()
#     print(content)
subprocess.run('docker build -t java:0.1 .', shell=True) 
file = open('file.txt','r').read()
output = subprocess.run('docker run -i java:0.1 > outputJAVA.txt', shell=True, capture_output=True,input=file.encode(),text=True)
# output = subprocess.run('docker run -it -v /tmp/python/:/python/data python:0.1 data.txt data/output2.txt', shell=True, capture_output=True)
print(output.stderr.decode('UTF-8'))
print(output.stdout.decode(('UTF-8')))
print(output)
